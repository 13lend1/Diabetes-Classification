...
sns.histplot(df['Age'],bins=20,shrink=0.8, edgecolor='black',color='brown')
...
# Build class columns for HDL, LDL, Creatinine and BUN before plotting
def goodFats(hdl):
    if hdl>1:
        return "Over 1"
    else:
...
# One combined histogram of classes for HDL, LDL, Creatinine and BUN with percentages on top
fig, axes = plt.subplots(2, 2, figsize=(14,10))

order_hdl=["Over 1","Under 1"]
sns.countplot(data=df,x="Good_fats", order=order_hdl, color='gold', edgecolor='black', ax=axes[0,0])
labeler(total, axes[0,0])
axes[0,0].set_title("HDL Classes")

order_ldl=["Under 3","Over 3"]
sns.countplot(data=df,x="Bad_fats", order=order_ldl, color='gold', edgecolor='black', ax=axes[0,1])
labeler(total, axes[0,1])
axes[0,1].set_title("LDL Classes")

order_cr=["Healthy","Unhealthy"]
sns.countplot(data=df,x="Cr_cat", order=order_cr, color='gold', edgecolor='black', ax=axes[1,0])
labeler(total, axes[1,0])
axes[1,0].set_title("Creatinine Classes")

order_bun=["Healthy","Unhealthy"]
sns.countplot(data=df,x="BUN_cat", order=order_bun, color='gold', edgecolor='black', ax=axes[1,1])
labeler(total, axes[1,1])
axes[1,1].set_title("BUN Classes")

plt.tight_layout()
plt.show()


# HDL: raw distribution + diabetes breakdown, combined
fig, axes = plt.subplots(1, 2, figsize=(14,6))
...
order=["Over 1","Under 1"]
sns.countplot(data=df,x="Good_fats",hue='Diabetes_Label', order=order,
              palette={'No Diabetes':'lightgray', 'Diabetes':'crimson'}, ax=axes[1])
grouped_labeler(axes[1],df,'Good_fats',order)
axes[1].set_title("HDL levels and Diabetes")

plt.tight_layout()
plt.show()


# LDL: raw distribution + diabetes breakdown, combined
fig, axes = plt.subplots(1, 2, figsize=(14,6))
...
order=["Under 3","Over 3"]
sns.countplot(data=df,x="Bad_fats",hue='Diabetes_Label', order=order,
              palette={'No Diabetes':'lightgray', 'Diabetes':'crimson'}, ax=axes[1])
axes[1].set_title("LDL levels and Diabetes")
grouped_labeler(axes[1],df,'Bad_fats',order)

plt.tight_layout()
plt.show()


# Creatinine: raw distribution + diabetes breakdown, combined
fig, axes = plt.subplots(1, 2, figsize=(14,6))
...
order=["Healthy","Unhealthy"]
sns.countplot(data=df,x="Cr_cat",hue='Diabetes_Label', order=order,
              palette={'No Diabetes':'lightgray', 'Diabetes':'crimson'}, ax=axes[1])
grouped_labeler(axes[1],df,'Cr_cat',order)
axes[1].set_title("Creatinine levels and Diabetes")

plt.tight_layout()
plt.show()


# BUN: raw distribution + diabetes breakdown, combined
fig, axes = plt.subplots(1, 2, figsize=(14,6))
...
order=["Healthy","Unhealthy"]
sns.countplot(data=df,x="BUN_cat",hue='Diabetes_Label', order=order,
              palette={'No Diabetes':'lightgray', 'Diabetes':'crimson'}, ax=axes[1])
grouped_labeler(axes[1],df,'BUN_cat',order)
axes[1].set_title("BUN levels and Diabetes")
